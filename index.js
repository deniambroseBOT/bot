...
const BotName = 'DABOT';
const wa = 'https://chat.whatsapp.com/DzRKpHZgXsK1416iXMGubM';
const ow = 'DENIAMBROSE';
const eror = '[ERROR]';
const nomorowner = '085747852482';
const ovo = '08';
const pulsa = '085747852482';
const dana = '085747852482';
const gopay = '08';
const instagram = 'http://www.instagram.com/deniambrose_';
...
conn.on('qr', qr => {
  ...
  console.log(`[ ${moment().format("HH:mm:ss")} ] SUBSCRIBE YT DENIAMBOSEE RAMLAN CHANNEL!`);
});
...
//CREATOR
    ...
    conn.sendMessage(id, `Nih nomor ownerku❤`, MessageType.text, { quoted: m })
  }
//DONASI
    ...
    axios.get(`https://docs-jojo.herokuapp.com/api/stalk?username=${sons}`).then((res) => {
      imageToBase64(res.data.graphql.user.profile_pic_url_hd)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, 'Wait a', MessageType.text, { quoted: m })
    ...
    axios.get(url)
      .then((result) => {
        ...
        imageToBase64(cowok)
          .then(
            (response) => {
              conn.sendMessage(id, 'Wait beb', MessageType.text, { quoted: m })
              var buf = Buffer.from(response, 'base64');
              conn.sendMessage(id, buf, MessageType.image, { caption: `Wait beb gantengan ownerku sih ini`, quoted: m })
            }
          )
          .catch(
            (error) => {
    ...
    axios.get(url)
      .then((result) => {
        ...
        imageToBase64(nimek)
          .then(
            (response) => {
              conn.sendMessage(id, 'Wait a btw walpaper random ye...', MessageType.text, { quoted: m })
              var buf = Buffer.from(response, 'base64');
              conn.sendMessage(id, buf, MessageType.image, { quoted: m })
            }
          )
    ...
    axios.get(`https://arugaz.my.id/api/textpro/blackpink?text=${teks}`)
      .then((res) => {
        imageToBase64(`https://arugaz.my.id/api/textpro/blackpink?text=${teks}`)
          .then(
            (ress) => {
              conn.sendMessage(id, 'Buset blackpink', MessageType.text, { quoted: m })
              var buf = Buffer.from(ress, 'base64')
              conn.sendMessage(id, buf, MessageType.image, { caption: `Nih a`, quoted: m })
            })
      })
...
//Tobz
//Mrfzvx12
//Deni ambrose dan All creator bot Whatsapp
